...
def plot_topology_map(file_path):
    ...
    n_per_seg = sfreq  # 每個分段的數據點數，這裡設置為 1 秒（1000 點）
    overlap = 0.5  # 重疊比例 50%

    eeg_data = eeg_data[10*sfreq : -10*sfreq,:19]
    ...
    channels_of_interest = ['Fp1', 'Fp2','C3', 'C4', 'P3', 'P4']
    # 繪製拓樸圖
    fig, axes = plt.subplots(3, 2, figsize=(6, 10))#,dpi=100
    fig.subplots_adjust(wspace=0.3)

    plt.suptitle('EEG Band Power Topography', fontsize=12)
    ...
    for i, (band, (fmin, fmax)) in enumerate(bands.items()):
        ...
        psd, freq = mne.time_frequency.psd_array_welch(raw_filiter.get_data(), sfreq=sfreq, fmin=fmin, fmax=fmax, n_fft=2048, n_per_seg=n_per_seg, n_overlap=int(n_per_seg * overlap))
    
        band_power = np.mean(psd, axis=1)
        # print(f'{band} band power:', band_power)

        im,_ = mne.viz.plot_topomap(band_power, raw.info, axes=axes[i,0], names= ch_names, show=False,cmap='jet') #,vlim=(2e-8, 1.5e-7)    
        plt.colorbar(im, ax=axes[i,0], fraction = 0.04)
        #繪製表格
        table_data = []
        for ch in channels_of_interest:
            ch_idx = ch_names.index(ch)
            value = band_power[ch_idx]
            formatted_value = f"{value:.2E}"  # 格式化成科學計數法，保留兩位有效數字
            table_data.append([ch,formatted_value])
    ...
    # c_bar = fig.colorbar(im, ax=axes[1,1], fraction=0.05,location='left',ticklocation='left')
    # # 调整颜色条的大小和字体大小
    # c_bar.ax.tick_params(labelsize=8)  # 设置颜色条刻度标签字体大小
    # c_bar.ax.yaxis.label.set_fontsize(8)  # 设置颜色条标签（如果有的话）的字体大小
    # c_bar.set_label('µV',loc='top')
    # # 设置颜色条宽度和高度
    ...
    plt.tight_layout(rect=[0, 0, 1, 1]) #左下右上間距
    # plt.savefig(f"{SAVE_DIR}/{' '.join('band power topography').title()}.png", dpi = 500)
    # plt.show()
    
    #計算頻帶比例
    band_ratios = calculate_band_ratios(eeg_data, sfreq)
    # print("Band Ratios:", band_ratios)

    return fig, band_ratios

def calculate_band_ratios(eeg_data, fs):  
    # 頻段過濾 
    bands = {
        'Theta' : (6, 8),
        'Alpha' : (8, 12),
    ...
    # 計算每個頻段的能量
    band_powers = {band: 0 for band in bands}

    for ch in range(eeg_data.shape[1]):
        # 計算Welch功率譜密度估計
        psd, freqs  = mne.time_frequency.psd_array_welch(eeg_data[:, ch], sfreq=fs, n_fft=2048, n_per_seg=fs, n_overlap=int(fs * 0.5))

        for band, (low, high) in bands.items():
            # 找到對應頻段範圍的索引
            idx_band = np.logical_and(freqs >= low, freqs <= high)
            # 計算頻段內的總能量
            # print(band, freqs[idx_band])
            band_powers[band] += np.sum(psd[idx_band])
    # print(band_powers)
    # 計算各頻段的比例
    total_power = sum(band_powers.values())
    band_ratios = {band: f'{(power / total_power*100):.2f}%' for band, power in band_powers.items()}
    ...
    for subject in SUBJECT_NAMES:
        ...
        for i, data_date in enumerate(data_dates):
            ...
            for i, (band, (fmin, fmax)) in enumerate(bands.items()):
                ...
                table.scale(1, 2)
                axes[i, 1].axis('off')
                c_bar = fig.colorbar(im, ax=axes[i,0], fraction=0.05,pad=0.05)
                # 调整颜色条的大小和字体大小
                # c_bar.ax.tick_params(labelsize=2)  # 设置颜色条刻度标签字体大小
                # c_bar.ax.yaxis.label.set_fontsize(8)  # 设置颜色条标签（如果有的话）的字体大小
            ...
            axes[1,0].text(-0.12,0, f'Alpha Band (8-12 Hz)',va = 'center', ha='center', rotation ='vertical')
            axes[2,0].text(-0.12,0, f'Beta Band (12-30 Hz)',va = 'center', ha='center', rotation ='vertical')
            plt.tight_layout(rect=[0, 0, 1, 1]) #左下右上間隔
            # plt.savefig(f"{SAVE_DIR}/{' '.join('band power topography').title()}.png", dpi = 500)
            # plt.show()
